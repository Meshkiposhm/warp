...
cfwarpIP() {
    if [[ ! -f "$PREFIX/bin/warpendpoint" ]]; then
        ...
        if [[ -n $cpu ]]; then
            curl -L -o warpendpoint -# --retry 2 https://raw.githubusercontent.com/Meshkiposhm/warp/main/endip/$cpu
...
generate() {
    if ! command -v wgcf &>/dev/null; then
        ...
        if [[ "$(uname -o)" == "Android" ]]; then
            ...
            if [[ -n $cpu ]]; then
                curl -o "$PREFIX/bin/wgcf" -L "https://raw.githubusercontent.com/Meshkiposhm/warp/main/endip/wgcf"
...
clear
echo -e "${cyan}By --> Peyman * Github.com/Meshkiposhm * ${rest}"
echo ""
echo -e "${purple}*********************${rest}"
